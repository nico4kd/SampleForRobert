import 'package:authentication_repository/authentication_repository.dart';
import 'package:core/core.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:flutter_template/app/common/common.dart';
import 'package:flutter_template/app/notifier/app_notifier.dart';
import 'package:flutter_template/app/widgets/widgets.dart';
import 'package:flutter_template/features/auth/profile/notifier/profile_notifier.dart';
import 'package:flutter_vector_icons/flutter_vector_icons.dart';
import 'package:formz/formz.dart';

part './widgets/profile_form.dart';

class ProfilePage extends ConsumerWidget {
  const ProfilePage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final status = ref.watch(
      profileNotifierProvider.select((value) => value.status),
    );

    final loading = ref.watch(
      profileNotifierProvider.select((value) => value.loading),
    );

    return Scaffold(
      // appBar: AppBar(
      //   title: const Text('COMPLETAR PERFIL'),
      // ),
      body: KeyboardDismissOnTap(
        child: Stack(
          children: [
            const _ProfileForm(),
            MyProgressIndicator(
              isVisible: status == FormzStatus.submissionInProgress || loading,
              text: loading ? 'Cargando' : 'Verificando',
            )
          ],
        ),
      ),
    );
  }
}
